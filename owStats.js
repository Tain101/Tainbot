...
module.exports = function() {
    ...
    this.owStats = function(message, args) {
        if (args[0] === "set") {
            if (!args[1]) {
                mybot.sendMessages("you need to give me your bnet! try again.");
                return false;
            }else{
                setOwStats(message.author, args[1]);
                mybot.sendMessages("your bnet has been saved!");
                return true;
            }
            ...
            if (owStatsJson.hasOwnProperty(user)) { //check if user already has linked their bnet
                mybot.sendMessages("https://www.overbuff.com/players/pc/" + owStatsJson[user]);
                return true;
            } else {
                mybot.sendMessages("you havent set up your bnet! try !owStat set [bnet]");
                return false;
            }
        }
    };
